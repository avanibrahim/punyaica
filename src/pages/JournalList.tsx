...
import { UploadCloud, List as ListIcon, CheckCircle2, Shield, CloudUpload, FileDown } from 'lucide-react';
import { BookOpen, FileText, Gauge, Cpu } from 'lucide-react';
import { motion, useAnimation, useInView } from 'framer-motion';

// ========================= Anim Variants & Wrapper =========================
...
export default function JournalList() {
  ...
  const paginated = useMemo(() => files.slice(0, currentPage * itemsPerPage), [files, currentPage]);
    ...
    <Layout>
      <div className="pb-0">
        ...
        <section className="relative overflow-hidden pt-4 pb-8 sm:pt-12 sm:pb-20 md:pt-16 md:pb-24">
          ...
          <div className="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid items-center gap-8 md:gap-12 md:grid-cols-2">
              {/* TEXT & CTA */}
              <Reveal as={motion.div} variants={fadeRight} className="order-2 md:order-1 text-center md:text-left space-y-10 md:space-y-7">
                {/* Badge HMI */}
                ...
                <Reveal as={motion.h1} variants={fadeUp}>
                ...
                <Reveal as={motion.p} variants={fadeUp}>
                ...
                {/* Feature bullets */}
                <Reveal as={motion.ul} variants={fadeUp}>
                  <ul className="mt-2 grid grid-cols-3 sm:grid-cols-3 gap-2 text-xs sm:text-sm text-emerald-700">
                    {[
                      ...
                      <Reveal as={motion.li} variants={fadeUp} delay={0.05 * i} key={txt}>
                        <li className="inline-flex items-center gap-2 justify-center md:justify-start">
                          <CheckCircle2 className="h-4 w-4 text-emerald-600" /> {txt}
                        </li>
                      </Reveal>
                    ))}
                  </ul>
              ...
              {/* IMAGE / LOGO */}
              <Reveal as={motion.div} variants={fadeLeft} className="order-1 md:order-2 flex justify-center md:justify-end">
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
              {stats.map(({ label, icon: Icon }, index) => (
                <Reveal as={motion.div} variants={scaleIn} delay={index * 0.05} key={index}>
                  <div className="text-center text-2xl md:text">
        ...
        <header id="list" className="text-center py-8 md:py-10">
          <Reveal as={motion.h2} variants={fadeUp}>
        ...
        <section className="z-20 px-4">
          <Reveal as={motion.div} variants={fadeUp} className="p-3 md:p-5">
            <SearchAndFilter
        ...
        <section className="px-4 mt-6">
          <Reveal as={motion.div} variants={scaleIn} className="rounded-2xl border border-emerald-200 bg-white shadow-sm">
          ...
          {hasMore && !isLoading && !error && (
            <Reveal as={motion.div} variants={fadeUp} className="text-center mt-6">
              <button
